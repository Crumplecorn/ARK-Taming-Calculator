		var tamingApp=angular.module('tamingApp', ['ngRoute', 'tamingControllers', 'ngCookies', 'ngAnimate']);

		tamingApp.config(['$routeProvider', '$locationProvider', function($routeProvider, $locationProvider) {
		...
		tamingApp.filter('secondsToDateTime', function() {
   		...
   		tamingApp.directive("select", function() {
		    return {
		    	restrict: "E",
		    	require: "?ngModel",
		    	scope: false,
		...
		tamingApp.directive('integer', function() {
			return {
				require: 'ngModel',
				link: function (scope, element, attrs, ngModel) {
		...
		tamingApp.directive('autocomplete', ['$window', '$parse', function($window, $parse) {
			return {
				require: ['ngModel'],
				scope: {
					ngModel: '=',
		...
		tamingApp.animation('.tableslide', [function() {
			return {
		...
		tamingApp.animation('.slide', [function() {
			return {
				addClass: function(element, className, doneFn) {
					if (className==="ng-hide") {
